...
module Clogs
  # The NAppGUI backend's stand-in for Clogs::UI: the enumerations the rest of
  # Clogs reaches for, and the two brush constructors `Background` uses.
  module UI
    # draw2d fills a polygon by the non-zero winding rule and offers no way to
    # ask for the alternate one. Clogs' shapes carry a fill mode all the same,
    # so `star` still asks for ALTERNATE -- it just gets WINDING.
    WINDING = 0
    ALTERNATE = 1

    # linecap_t and linejoin_t, in their declared order.
    CAP_FLAT = 0
    CAP_ROUND = 1
    CAP_SQUARE = 2
    JOIN_MITER = 0
    ...
    # `background red..blue`. draw2d has a real linear gradient fill; the stops
    # are carried until the painter knows the box to run them across.
